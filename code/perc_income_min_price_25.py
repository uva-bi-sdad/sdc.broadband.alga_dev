...
def perc_income_min_price_25():
    export_dir = (
        "../data/Affordability/Percentage of income for good internet/data/distribution"
    )
    ...
    for acs_f, bbn_f in pbar:
        ...
        bbn_df_u_25 = bbn_df[
            (bbn_df["down_up"] == "Download") & (bbn_df["speed_value"] >= 25)
        ]
        bdf = (
            bbn_df_u_25.groupby(["geoid", "year_parsed"])["price"]
            .agg("min")
            .to_frame()
            .reset_index()
        )
        ...
        bg_df["measure"] = "perc_income_min_price_25"
        bg_df["value"] = (bg_df["price"] * 12) / bg_df["B19013_001E"] * 100
