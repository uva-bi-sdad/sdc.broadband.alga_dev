...
MEASURE_NAME = "perc_hh_with_cable_fiber_dsl"
...
def perc_hh_with_cable_fiber_dsl():
    export_dir = (
        "../data/Adoption/Households with cable, fiber optic, or DSL/data/distribution"
    )
    c_files = sorted(
    ...
    d_files = sorted(
        pathlib.Path("../sdc.broadband.acs/data/distribution/B28002_007").glob(
            "*.csv.xz"
        )
    )
    ...
    for c_f, d_f in zip(c_files, d_files):
        ...
        comp_df = comp_df[
            (comp_df["B28002_001E"] >= 0) & (comp_df["B28002_001E"].notnull())
        ].copy()
        comp_df = comp_df.rename(columns={"GEOID21": "geoid"})
        comp_df["year"] = 2021
        comp_df = (
            comp_df.groupby(["geoid", "year"])["B28002_001E"]
            .agg("min")
            .to_frame()
            .reset_index()
        )
        ...
        cd_df = cd_df[
            (cd_df["B28002_007E"] >= 0) & (cd_df["B28002_007E"].notnull())
        ].copy()
        cd_df = cd_df.rename(columns={"GEOID21": "geoid"})
        cd_df["year"] = 2021
        cd_df = (
            cd_df.groupby(["geoid", "year"])["B28002_007E"]
            .agg("min")
            .to_frame()
            .reset_index()
        )
        cd_df = cd_df[cd_df["B28002_007E"] > 0]
        bg_df = pd.merge(comp_df, cd_df, on=["geoid", "year"])
        bg_df["measure"] = MEASURE_NAME
        bg_df["value"] = bg_df["B28002_007E"] / bg_df["B28002_001E"] * 100
