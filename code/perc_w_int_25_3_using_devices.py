...
MEASURE_NAME = "perc_w_int_25_3_using_devices"
...
def generate_higher_geo(df, geoid_length=11):
    ...
    for rt in df[region_type].unique():
        ...
        pdf["weighted_pop"] = (pdf["B28002_001E"] - pdf["B28002_013E"]) / (
            pdf["B28002_001E"].sum() - pdf["B28002_013E"].sum()
        )
        pdf["value"] = (
...
def perc_w_int_25_3_using_devices():
    export_dir = (
        "../data/Accessibility/Percent Good (internet-connected)/data/distribution"
    )
    ...
    pbar = tqdm(zip(ookla_files, acs_1_files, acs_13_files))
    for ookla_f, acs1_f, acs13_f in pbar:
        ...
        bg_df["upload_prob"] = bg_df["geoid"].apply(
            lambda x: calculate_cpdf(
                ookla_df[ookla_df["geoid"].str[: len(x)] == x], "avg_u_mbps", 3
            )
        )
        bg_df["download_prob"] = bg_df["geoid"].apply(
            lambda x: calculate_cpdf(
                ookla_df[ookla_df["geoid"].str[: len(x)] == x], "avg_d_mbps", 25
            )
        )
