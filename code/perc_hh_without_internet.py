...
MEASURE_NAME = "perc_hh_without_internet"
...
def perc_hh_without_internet():
    export_dir = "../data/Adoption/Households without internet access/data/distribution"
    c_files = sorted(
    ...
    for c_f, d_f in zip(c_files, d_files):
        ...
        cd_df = cd_df[
            (cd_df["B28002_013E"] >= 0) & (cd_df["B28002_013E"].notnull())
        ].copy()
        cd_df = cd_df.rename(columns={"GEOID21": "geoid"})
        cd_df["year"] = 2021
        cd_df = (
            cd_df.groupby(["geoid", "year"])["B28002_013E"]
            .agg("min")
            .to_frame()
            .reset_index()
        )
        cd_df = cd_df[cd_df["B28002_013E"] > 0]
        bg_df = pd.merge(comp_df, cd_df, on=["geoid", "year"])
        bg_df["measure"] = MEASURE_NAME
        bg_df["value"] = bg_df["B28002_013E"] / bg_df["B28002_001E"] * 100
