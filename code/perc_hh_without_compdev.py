...
MEASURE_NAME = "perc_hh_without_compdev"
...
def perc_hh_without_compdev():
    export_dir = "../data/Adoption/Households without a computer/data/distribution"
    c_files = sorted(
        pathlib.Path("../sdc.broadband.acs/data/distribution/B28001_001").glob(
            "*.csv.xz"
        )
    )
    d_files = sorted(
        pathlib.Path("../sdc.broadband.acs/data/distribution/B28001_002").glob(
            "*.csv.xz"
        )
    )
    ...
    for c_f, d_f in zip(c_files, d_files):
        ...
        comp_df = comp_df[
            (comp_df["B28001_001E"] >= 0) & (comp_df["B28001_001E"].notnull())
        ].copy()
        comp_df = comp_df.rename(columns={"GEOID21": "geoid"})
        comp_df["year"] = 2021
        comp_df = (
            comp_df.groupby(["geoid", "year"])["B28001_001E"]
            .agg("min")
            .to_frame()
            .reset_index()
        )
        ...
        cd_df = cd_df[
            (cd_df["B28001_002E"] >= 0) & (cd_df["B28001_002E"].notnull())
        ].copy()
        cd_df = cd_df.rename(columns={"GEOID21": "geoid"})
        cd_df["year"] = 2021
        cd_df = (
            cd_df.groupby(["geoid", "year"])["B28001_002E"]
            .agg("min")
            .to_frame()
            .reset_index()
        )
        cd_df = cd_df[cd_df["B28001_002E"] > 0]
        bg_df = pd.merge(comp_df, cd_df, on=["geoid", "year"])
        bg_df["measure"] = MEASURE_NAME
        bg_df["value"] = (
            (bg_df["B28001_001E"] - bg_df["B28001_002E"]) / bg_df["B28001_001E"] * 100
        )
